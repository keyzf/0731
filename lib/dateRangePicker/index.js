...
var DateRangePicker = React.createClass({
  ...
  _onEndDateChange: function _onEndDateChange(date) {
    var self = this;
    if (typeof date === 'undefined' || date == null) {
  ...
  _closeOnEndSelect: function _closeOnEndSelect() {
    this.setState({
      endChecked: true
    }, function () {
      if (this.state.startChecked && this.state.endChecked && !this.props.showTime) {
  ...
  _hideCalendar: function _hideCalendar(confirm) {
    var that = this;
    if (this.state.showCalendar) {
      this.setState({
        showCalendar: false
        ...
        if (typeof that.props.onChange === 'function') {
          ...
          if (confirm === true) {
            if (that.props.minDate && new Date(startDateTimeString) >= new Date(that.props.minDate) || !that.props.minDate) {

              if (!that.state.startValue) {
                that.state.startValue = this.props.showTime ? startDateTimeString : dateString;
              }
            }
            if (that.props.maxDate && new Date(endDateTimeString) <= new Date(that.props.maxDate) || !that.props.maxDate) {
  ...
  _onEndTimeChange: function _onEndTimeChange(value) {
  ...
  _createPopup: function _createPopup() {
    ...
    ReactDom.render(this.state.showCalendar ? React.createElement(
      'div',
      null,
      React.createElement('div', { className: 'date-range-popup-mask', onClick: this._hideCalendar }),
      React.createElement(
        'div',
          ...
          React.createElement(
            ...
            this.props.showTime ? React.createElement(
              ...
              React.createElement(
                ...
                React.createElement(
                  'div',
                  { onClick: this._onConfirm, className: 'btn btn-rounded close-button' },
                  '\u786E\u5B9A'
                )
              )
            ) : null
  ...
  render: function render() {
    ...
    return React.createElement(
      'div',
      { className: classnames({ 'date-range-picker': true }, this.props.className), style: assign({ width: 215, whiteSpace: 'nowrap' }, this.props.style) },
      React.createElement(
        'div',
        { ref: 'canlendar_input_ref' },
        this.state.showCalendar || this.state.options.length <= 1 ? React.createElement(
          'div',
          { className: 'calendar-input' },
          React.createElement(
            'div',
            { className: 'button', onClick: this._calendarInputClick, style: assign({ width: '100%', whiteSpace: 'nowrap' }, this.props.style) },
            (this.state.startValue ? this.state.startValue : '开始时间') + ' - ' + (this.state.endValue ? this.state.endValue : '结束时间')
          )
        ) : React.createElement(Select, { name: name, options: this.state.options, defaultValue: this.state.selectValue, onChange: this._onChange, style: assign({ width: '100%', whiteSpace: 'nowrap' }, this.props.style) })
      )
    );
  }
});
