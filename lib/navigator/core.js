...
var Core = React.createClass({
  ...
  _parseDataByUrl: function _parseDataByUrl(data, params, url, init, onEqualFunc) {
  ...
  _compare: function _compare(url, data, init, params, onEqualFunc) {
    ...
    switch (this.props.dataLayerIndex) {
      case 2:
        for (var key in data) {
          if (_typeof(data[key].value) === 'object' && data[key].value != null) {
      ...
      case 3:
        for (var key in data) {
          for (var k in data[key].value) {
            if (_typeof(data[key].value[k].value) === 'object' && data[key].value[k].value != null) {
