...
module.exports.parseDataByUrl = function (data, params, url, init, onEqualFunc) {
  var parseData = function parseData(data, dataLayerIndex) {
    var config = null;
  ...
  var menuEle = Object.keys(data).map(function (key, i) {
    ...
    if (typeof value != 'undefined' && value != null && value !== '' && params.dataLayerRange != 1) {
      var ele = Object.keys(value).map(function (k, ii) {
        ...
        var className = 'layer2 ' + (v[k].className || '');
        var icon = v[k].icon ? v[k].icon : v[k].iconClassName ? React.createElement('i', { className: v[k].iconClassName }) : null;
        ...
        return React.createElement(
          'li',
          { key: i + '-' + ii, className: className, style: { paddingTop: 0 } },
          React.createElement(
            'a',
    ...
    var _handleClick = function _handleClick(open) {
      ...
      if (params.alwaysOpen) {} else if (typeof data[key].value === 'undefined' || data[key].value == null || data[key].value.length === 0 || params.dataLayerRange == 1) {
        data[key].open = false;
      } else {
        if (open === true) {
          data[key].open = true;
    ...
    var icon = data[key].icon ? data[key].icon : data[key].iconClassName ? React.createElement('i', { className: data[key].iconClassName }) : null;
