...
var Breadcrumb = React.createClass({
   ...
   */
	...
	_arrayIncludeUrl: function _arrayIncludeUrl(array, url) {
		var include = false;
	...
	_pushToArray: function _pushToArray(array, data, url) {
		for (var key in data) {
	...
	_createBreadcrumbArray: function _createBreadcrumbArray(init) {
		...
		if (typeof this.props.defaultItem != 'undefined' && this.props.defaultItem != null) {
			array.unshift(this.props.defaultItem);
		}
	...
	_createOnClickFunction: function _createOnClickFunction(url, alias, text) {
		var that = this;
		return function () {
	...
	render: function render() {
		...
		for (var key in array) {
			if (array[key] !== array[array.length - 1]) {
				links.push(React.createElement(
					...
					React.createElement(
						'span',
						{ onClick: this._createOnClickFunction(array[key].url, array[key].alias, array[key].text), style: { cursor: 'pointer' } },
						array[key].text
					),
					React.createElement(
						'span',
		...
		return React.createElement(
			'ul',
			{ className: classnames({ 'breadcrumb': true }, this.props.className), style: assign({}, this.props.style) },
			links
		);
	}
});
