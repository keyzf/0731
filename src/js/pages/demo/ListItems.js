...
module.exports = React.createClass({
   getInitialState:function(){
    ...
    _update: function() {
        var data = DemoStore.getData(StoreConfig.STORE_DEMO_ITEM_LIST_DATA);
    ...
    _getItemList: function() {
        DemoAction.getOrderList(this.state.limit,this.state.pageIndex);
    },
    _edit: function(index,rowData) {
        console.log('-edit',index,rowData)
        this.setState({item_id:rowData.item_id,editItem:true},function(){
            ...
            <div className='content'>
                ...
                <div className ='content-body'>
                    <RaTable
                        ...
                        deleteRow={false}
                        action={[
                          {content:(<i className="iconfont icon-bianji" title="修改"></i>), action:this._edit},
                          {content:"删除", action:this._delete}
                        ]}>

                        <RaTable.Column dataField="orderNum" sort={true} >编号</RaTable.Column>
                        <RaTable.Column dataField="name" >用户</RaTable.Column>
                        <RaTable.Column dataField="description">评价商品</RaTable.Column>
                        <RaTable.Column dataField="time" align="center">日期</RaTable.Column>
                        </RaTable>
                </div>
                {this.state.editItem ?
