...
function Validator(rule, value) {
    var _regulars = {
        ...
        number: /^[-]?\d+((\.\d+)|(\d*))$/,
        int: /^[-]?\d+$/,//整数
        naturalNumber:/^(0|[1-9]\d*)$/,//自然数，正整数和0
        float: /^[-]?\d+\.\d+$/,
...
module.exports = {
    ...
    validateValue: function (rule,value) {
        return new Validator({type:rule}, value).validate();
    },
    linkState: function (ctx, path, eventProp) {
        var updateIn = function (obj, path, value) {
            var current = obj;
    ...
    submitDataWithFile: function (params) {
        var self = this;
        ...
        var options = {
            ...
            uploading: function (progress) {
                if (progress.total)
                    console.log('loading...', parseInt(progress.loaded / progress.total * 100) + '%');
            },
        ...
        var fileUpload = ReactDom.render((<FileUpload options={options}/>), div);
