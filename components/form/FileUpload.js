...
const FileUpload = React.createClass({
  ...
  commonUpload() {
    ...
    /*清除input的值*/
    if (this.refs['ajax_upload_file_input']) this.refs['ajax_upload_file_input'].value = ''
  },
    ...
    if (this.isIE) {
      ...
      render = (
        <div className={this.props.className} style={this.props.style}>
          {this.state.before}
          <div onClick={this.commonChooseFile} style={{overflow: 'hidden',postion: 'relative',display: this.wrapperDisplay}}>
            {this.state.chooseBtn}
          </div>
          {this.state.middle}
          <div onClick={this.commonUpload} style={{ overflow: 'hidden', postion: 'relative', display: this.chooseAndUpload ? 'none' : this.wrapperDisplay }}>
    ...
    function _insertIEForm (formArr, i) {
      ...
      formArr.push((
        <form
          ...
          {this.state.middle}
          <div style={{ overflow: 'hidden', position: 'relative', display: this.chooseAndUpload ? 'none' : this.wrapperDisplay }}>
            {this.state.uploadBtn}
            <input type='submit' style={{ position: 'absolute', left: 0, top: 0, fontSize: '50px', width: '200px', opacity: 0 }} />
          </div>
          {this.state.after}
        </form>
        ))
      formArr.push((
        <iframe
          ...
          style={{ display: 'none', width: 0, height: 0, margin: 0, border: 0 }}>
        </iframe>
        ))
    }
  }
