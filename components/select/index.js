...
var Select = React.createClass({
          ...
          for (var index in that.props.options) {
            if (deepEqual(value, that.props.options[index][that.props.displayValue])) {
              multiselectArray.push({
                [that.props.displayKey]: that.props.options[index][that.props.displayKey],
                [that.props.displayValue]: that.props.options[index][that.props.displayValue]
              })
            }
          }
        })
  ...
  _createDrop: function () {
    ...
    return optionEle.filter(function (item) { return item }).length ? optionEle : noValue
  },
  render: function () {
    var that = this
    var drop = null
    ...
    // if (this.state.open) {
    drop = (<div className='select2-drop select2-drop-active' ref='drop' style={this.state.open ? {display: 'block'} : {display: 'none'}}>
    ...
    if (this.props.multiselect) {
      ...
      trigger = (
        <a
          ...
          {this.state.name}<span className='select2-arrow'><b></b></span></a>
      )
    }

    return (
      <div ref='select_div' className={className} style={assign({}, this.props.style)}>
        {trigger}
        {drop}
      </div>
    )
